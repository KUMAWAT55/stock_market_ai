# Day 6: HTTP and REST Fundamentals

## Goal
Methods, status codes, query params, and JSON payloads

## App Flow Focus
How frontend polling talks to FastAPI endpoints

## Read in This Repo
- README.md, api/main.py

## Hands-On Work (90 to 120 mins)
1. Read the target files and write 5 to 10 bullet notes in your own words.
2. Complete this task: Use curl for 5 endpoints: /health, /market/symbols, /engine/status, /historical/ensure/status, /dashboard/snapshot
3. Capture one blocker and how you resolved it.

## End-of-Day Output
- Collected request-response examples
- 5 key things learned.
- 2 open questions for tomorrow.

## Self-Check
- I can explain today's flow focus in simple language.
- I can point to exact files where this behavior lives.
- I can run at least one command/API call related to today's topic.

## Notes Template

```txt
What confused me:
What clicked today:
What I changed/tested:
What to revise this weekend:
```
